...
void CGameObject::GetAABBCollisions(
	vector<LPGAMEOBJECT>* coObjects,
	vector<LPCOLLISIONEVENT>& coEvents)
{
	for (UINT i = 0; i < coObjects->size(); i++)
	{
		LPCOLLISIONEVENT e = new CCollisionEvent(0, 0, 0);
		if (checkAABB(coObjects->at(i))) // kiểm tra va chạm bằng AABB trước
		{
			e->obj = coObjects->at(i);
			coEvents.push_back(e);
			continue;
		}
